...
fn main() -> Result<()> {
    ...
    'o: loop {
        ...
        'fall: loop {
            ...
            if can_fall(s.0, s.1 + 1, &sand, &rock) {
                s.1 += 1;
            } else if can_fall(s.0 - 1, s.1 + 1, &sand, &rock) {
                s.1 += 1;
                s.0 -= 1;
            } else if can_fall(s.0 + 1, s.1 + 1, &sand, &rock) {
                s.1 += 1;
...
fn can_fall(x: u32, y: u32, sand: &HashSet<(u32, u32)>, rock: &HashSet<(u32, u32)>) -> bool {
    !sand.contains(&(x, y)) && !rock.contains(&(x, y))
}
